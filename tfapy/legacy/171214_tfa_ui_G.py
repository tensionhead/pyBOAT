...
class MainWindow(QMainWindow):
    ...
    def home(self):
        ...
        self.openFileButton.move(50,50)
...
class DataViewer(QWidget):
    ...
    def wlet_ana(self):
        ...
        self.wletWindow[self.i] = WaveletAnalyzer(signal=signal, dt=self.dt, T_min= self.T_min_value, T_max= self.T_max_value, position= self.i, signal_id =self.id, step_num= self.step_num_value)
...
class TimeSeriesWindowView(QWidget):
    ...
    def initUI(self):
        ...
        self.mplot = TimeSeriesPlotView(self)#, width = 6,height = 12)
        self.mplot.move(0,0)
        self.setSizePolicy(QSizePolicy.Expanding,QSizePolicy.Expanding)
        self.updateGeometry()
        # print('TimeWidget Size Policy', self.sizePolicy())
...
class TimeSeriesPlotView(FigureCanvas):
    ...
    def mpl_update(self, tvec, signal,trend, plot_raw, plot_trend, plot_detrended, clear = True, time_label = 'min'):
        ...
        if plot_detrended:
            ax2 = self.ax1.twinx()
            ax2.plot(tvec, signal - trend,'-', color = 'k',lw = 1.5, alpha = 0.7) 
