...
class DataViewer(QWidget):
    ...
    def doPlot(self):
        ...
        try:
            ...
            self.plotWindow.update(self.tvec, self.raw_signal, self.trend, plot_raw= self.plot_raw, plot_trend=self.plot_trend, plot_detrended=self.plot_detrended)
    ...
    def wlet_ana(self):
        ...
        self.wletWindow[self.i] = WavletAnalyzer(signal=signal, dt=self.dt, T_min= self.T_min_value, T_max= self.T_max_value, position= self.i, signal_id =self.id)
...
class WavletAnalyzer(QWidget):
    def __init__(self, signal, dt, T_min, T_max, position, signal_id):
        super().__init__()

        self.setWindowTitle('WavletAnalyzer - '+str(signal_id))
        self.setGeometry(510+position,30+position,600,700)

        periods=np.linspace(T_min, T_max, 100)
        print (periods[-1])
        ...
        waveletPlot = FrequencyWindow(signal, dt, periods)
        ...
        main_layout.addWidget(waveletPlot, 2,0,2,3)
...
class FrequencyWindow(QWidget):
    def __init__(self, signal,dt,periods):
        super().__init__()
        plot = FrequencyPlot(self)
        
        plot.modulus_plot( signal, dt, periods)
        #self.show()
