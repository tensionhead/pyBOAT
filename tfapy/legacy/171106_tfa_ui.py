...
class DataViewer(QWidget):
    ...
    def initUI(self):
        ...
        cb_trend.toggle()
        cb_detrend.toggle()
        cb_raw.stateChanged.connect(self.action)
        cb_trend.stateChanged.connect(self.action)
        cb_detrend.stateChanged.connect(self.action)
...
class WavletAnalyzer(QWidget):
    ...
    def initUI(self):
        ...
        self.setGeometry(510,310,600,500)
        ...
        waveletPlot = wl.compute_spectrum(signal, dt, periods)
        print (waveletPlot)
        main_layout_v = QVBoxLayout()
...
class TimeSeriesWindow_2(QWidget):
    ...
    def initUI(self):
        ...
        self.mplot = TimeSeriesPlot_2(self)
        self.mplot.move(0,0)
...
class TimeSeriesPlot_2(FigureCanvas):
    ...
    def mpl_update(self, tvec, signal, clear = True):
        ...
        if ptitle:
            self.ax1.set_title(ptitle)
        
        self.ax1.plot(tvec,signal,lw = 1.5, color = 'royalblue',alpha = 0.8)

        # plot the trend
        self.ax1.plot(tvec,signal,color = 'orange',lw = 1.5) 
...
class TimeSeriesWindow(QWidget):
    ...
    def initUI(self):
        #self.setWindowTitle(title)
        self.setGeometry(10,10,440,300)
        self.mplot = TimeSeriesPlot(self)
        self.mplot.move(0,0)
...
def plot_signal_trend(signal,trend, tvec,dt, fig_num = 1, ptitle = None,time_label = 'min'):
