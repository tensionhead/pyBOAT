...
###################################
    def Viewer_Ini (self):
        ...
        # make new DataViewer
        self.view[self.nViewers] = DataViewer()
        
        self.view[self.nViewers].make_connection(self.dh) 
...
class DataLoader(QWidget):
    ...
    def emit_values(self):
        for name, values in self.raw_df.iteritems():
            self.signal_dic[name] = values
            
        self.DataTransfer.emit(self.signal_dic)
...
class DataHandler(QWidget):
    ...
    def make_connection(self, signal_object):        ###########################
        signal_object.DataTransfer.connect(self.get_signal) #######################
...
class DataViewer(QWidget):
    ...
    # not needed, check checkboxes directly!
    ...
    def data_input(self):
        ...
        # self.table = QTableWidget() # doesn't show the table
    ...
    def set_periods(self):
        ...
        if np.size(self.raw_signal) > 1: # data prep didn't succeed
            ...
            self.T_max.insert(str(int(self.dt*0.5*len(self.raw_signal))))
    ...
    def doPlot(self):
        ...
        try:
            # no trend without T_cut_off value is set by user
            if self.T_c.isModified():
                try:
                    T_c = float(self.T_c.text())
                except ValueError:
                    self.outofBoundary = Error('Cut off period must be numeric!','Value Error')
                    return # don't plot
            ...
            self.noDataSelected = Error('No time series selected!','Missing data')
