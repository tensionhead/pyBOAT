...
class DataViewer(QWidget):
    ...
    def initUI(self):
        ...
        wlet_button_layout_h = QHBoxLayout()        wletButton.clicked.connect(self.wlet_ana)
...
class WaveletAnalyzer(QWidget):
    def __init__(self, signal, dt, T_min, T_max, position, signal_id, step_num):
        ...
        self.waveletPlot.plot_signal_modulus(tvec, signal,self.modulus,periods)
    ...
    def initUI(self, position):
        ...
        ridgeButton = QPushButton('Detect Ridge!', self)
        ridgeButton.clicked.connect(self.do_ridge_detection)
        ...
        self.power_thresh_tb = QLineEdit('0')
        
        options_box = QGroupBox('')
        #options_box.setSizePolicy(QSizePolicy.Minimum,QSizePolicy.Preferred)
        #options_layout= QFormLayout()
        ...
        options_box.setLayout(box_layout)
        
        main_layout = QGridLayout()
        main_layout.addWidget(self.waveletPlot, 0,0,5,5)
        main_layout.addWidget(options_box,5,0)
        self.setLayout(main_layout)
        
        self.show()

    # merge with F version
    def save_out(self):
        self.waveletPlot.save(self.signal_id)

    def do_ridge_detection(self):
        print(self.RidgeMSelection.currentText())
        print(self.power_thresh_tb.text())
        
        try:
            power_thresh = float(self.power_thresh_tb.text())
        except ValueError:
            print('except')
        ...
        if 'max' in self.RidgeMSelection.currentText():
            rdata = wl.get_maxRidge(self.modulus,self.wlet,self.periods,self.tvec,Thresh = power_thresh, smoothing = True, win_len = 17)

        if 'anneal' in self.RidgeMSelection.currentText():
            rdata = None

        if not rdata:
            self.e = Error('No ridge found..check spectrum/power threshold')
            return
