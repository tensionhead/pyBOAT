...
class DataHandler(QWidget):
    '''Handels all data that is loaded in (via DataLoader) or generated '''
    #Add signal to be emitted to pass on datasets
...
class DataViewer(QWidget):
    ...
    def wlet_ana(self):
        ...
        self.anaWindows[self.i] = WaveletAnalyzer(data=signal, dt=self.dt, T_min= self.T_min_value, T_max= self.T_max_value, position= self.i, signal_id =self.signal_id, step_num= self.step_num_value, v_max = self.v_max_value, time_unit= self.time_unit)
...
class FourierCanvas(FigureCanvas):
    ...
    def plot_spectrum(self,data,dt, time_unit):
        ...
        rf = rfft(data) # positive frequencies
        
        print(N,dt,df)
        print(len(fft_freqs),len(rf))
        
        print('Fourier power: ', max(np.abs(rf)))
        fpower = np.abs(rf)/np.var(data)
...
class WaveletAnalyzer(QWidget):
    ...
    def __init__(self, data, dt, T_min, T_max, position, signal_id, step_num, v_max, time_unit):
        super().__init__()
        ...
        # Plot input data
        self.tvec = np.arange(0,len(data)*dt,dt)
        #self.tvec = tvec
        ...
        #=============Compute Spectrum============================
        self.modulus, self.wlet = wl.compute_spectrum(self.data, dt, self.periods)
        #========================================================
    ...
    def initUI(self, position):
        ...
        self.waveletPlot.plot_signal_modulus(self.tvec, self.data,self.modulus,self.periods, self.v_max, self.time_unit)
        #-----------------------------------------------------------------------
...
class SynthViewer (DataViewer):
    '''Class to generate synthetic datasets
    Inherits DataViewer and is modified in such a way that the part about the dataset loading is replaced.
    Data is added using the helper function self.gen_func = synth_signal1'''
    def __init__(self):
        super().__init__()
        self.default_para_dic = {'T' : 900, 'amp' : 6, 'per' : 70, 'sigma' : 2, 'slope' : -10.} # default parameter dictionary containing all parmeters for the creation of the synthetic data
        self.gen_func = synth_signal1 # Which helper function is used
        self.raw_data= pd.DataFrame() # DataFrame into which generated data is appended
        self.series_ids = [] # List to which dataseries ids are appended
    ...
    def data_input(self):
        ...
        self.parameter = NumericParameterDialog(self.default_para_dic) # Creates Dialog with text boxes default parameters and parameter labels
        
        addButton=QPushButton('Add data', self) #Button to add synthetic data with current parameters from textboxes
        addButton.clicked.connect(self.add_button)
        
        self.table = QGroupBox('Test') #Creates box #table needs to be QWidget
        #Layout to which all widgets are added and then displayed
    ...
    def add_button (self):
        pdic = self.parameter.read() # Reads out current numbers
        tvec, data = self.gen_func( **pdic) # Uses helper function and gets back a time vector and a dataseries (data)
        self.raw_data[str(self.count)+'_SynSig_{}'.format(pdic)] = data
        self.series_ids.append(str(self.count) +'_SynSig_{}'.format(pdic))
        self.count +=1
        self.fillSignalBox() # Calls function that fills in dataseries ids to the combo box from which dataseries is selected for further analysis
...
class NumericParameterDialog(QDialog):
    '''Class Dialog that contains text boxes labeled with parameter names.
    Takes a dictionary with parameter names and default values that will befilled into the text boxes as default.
    Used by SynthViewer
    '''
    ...
    def createFormGroupBox(self):
        '''Creates layout with textboxes and labels'''
    ...
    def read(self):
        '''Reads out numbers from text boxes and returns a dictonary containing recent numbers with parameter names'''
...
class Error(QWidget):
    '''General error class. Recieves error window titel and error meessage and opens an error window showing it.'''
    def __init__(self, message,title):
        super().__init__()
        self.message = message #Displayed error message
        self.title = title #Title of the error window
        self.initUI()
...
def synth_signal1(T, amp, per, sigma, slope):  
    '''T: time
        ...
        sigma: error
        slope: slope'''
    tvec = np.arange(T) #creates time vector
    trend = slope*tvec**2/tvec[-1]**2*amp #creates artifical trend using supplied slope
    noise = np.random.normal(0,sigma, len(tvec)) # creates random noise around sigma
    sin = amp*np.sin(2*np.pi/per*tvec)+noise+trend # creates synthetic signal with error and trend
